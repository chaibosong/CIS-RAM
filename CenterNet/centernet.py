...
from CenterNet.utils.utils import (cvtColor, get_classes, preprocess_input, resize_image,
                         show_config)
from CenterNet.utils.utils_bbox import decode_bbox, postprocess
...
class CenterNet(object):
    _defaults = {
        ...
        # "model_path": 'logs/ep030-loss5.028-val_loss4.573.pth',
    ...
    def __init__(self, **kwargs):
        ...
        with open('model_data/pth_name.txt', 'r') as f:
            ...
            f.close()
        self.model_path = os.path.join('weights/singlemodel', self.model_path)
            # ---------------------------------------------------#
        #   计算总的类的数量
        # ---------------------------------------------------#
    ...
    #   检测图片
    # ---------------------------------------------------#
    def detect_image(self, image, crop=False, count=False,confidence=0.15):
        # ---------------------------------------------------#
        #   计算输入图片的高和宽
        # ---------------------------------------------------#
    ...
    def create_fm(self, image, crop=False, count=False):
        # ---------------------------------------------------#
        #   计算输入图片的高和宽
        # ---------------------------------------------------#
        image_shape = np.array(np.shape(image)[0:2])
