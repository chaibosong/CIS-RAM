...
import torch
from CenterNet.nets.centernet_training import focal_loss, reg_l1_loss
from tqdm import tqdm

from CenterNet.utils.utils import get_lr
...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, backbone, save_period, save_dir, local_rank=0):
    ...
    if local_rank == 0:
        ...
        if len(loss_history.val_loss) <= 1 or (val_loss / epoch_step_val) <= min(loss_history.val_loss):
            print('Save best model to singlemodel_best_weights.pth')
            torch.save(model.state_dict(), os.path.join(save_dir, "singlemodel_best_weights.pth"))
            shutil.copy(os.path.join(save_dir, "singlemodel_best_weights.pth"), 'weights/singlemodel/singlemodel_best_weights.pth')
        torch.save(model.state_dict(), os.path.join(save_dir, "singlemodel_last_weights.pth"))
        shutil.copy(os.path.join(save_dir, "singlemodel_last_weights.pth"), 'weights/singlemodel/singlemodel_last_weights.pth')