...
class EvalCallback():
    def __init__(self, net, backbone, input_shape, class_names, num_classes, val_lines, hrrp_lines, label_lines, log_dir, cuda, \
    ...
    def get_map_txt(self, image_id, image, class_names, map_out_path):
        ...
        for lable_line in self.label_lines:
            if lable_line.split(',')[0].split('.')[0] == image_id.split('/')[-1]:
        ...
        with torch.no_grad():
            ...
            xys = torch.from_numpy(np.expand_dims(np.array(batch_xys), 0)).type(torch.FloatTensor)
            hrrps = torch.from_numpy(np.expand_dims(np.array(batch_hrrps), 0)).type(torch.FloatTensor)
