...
from FusionNet._utils.utils import (cvtColor, get_classes, preprocess_input, resize_image,
                          show_config)
from FusionNet._utils.utils_bbox import decode_bbox, postprocess
...
class CenterNet(object):
    _defaults = {
        ...
        "model_path": './logs/best_car_building.pth',
    ...
    def __init__(self, **kwargs):
        ...
        with open('model_data/pth_name.txt', 'r') as f:
            self.model_path = f.readline()
            f.close()
        self.model_path = os.path.join('weights/multimodel',self.model_path)
    ...
    #   检测图片
    # ---------------------------------------------------#
    def detect_image(self, image, xys, hrrps, crop=False, count=False,confidence=0.15):
        # ---------------------------------------------------#
        #   计算输入图片的高和宽
        # ---------------------------------------------------#
        ...
        with torch.no_grad():
            ...
            xys = torch.from_numpy(np.expand_dims(np.array(xys), 0)).type(torch.FloatTensor)
            hrrps = torch.from_numpy(np.expand_dims(np.array(hrrps), 0)).type(torch.FloatTensor)
            ...
            # ---------------------------------------------------------#
            #xys = None
            outputs = self.net(images, xys, hrrps)
            ...
            #   利用预测结果进行解码
            # -----------------------------------------------------------#
            outputs = decode_bbox(outputs[0], outputs[1], outputs[2], confidence, self.cuda)
