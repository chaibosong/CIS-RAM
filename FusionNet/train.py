...
from _utils.callbacks import LossHistory
from _utils.callbacks import EvalCallback, LossHistory
from _utils.dataloader import CenternetDataset, centernet_dataset_collate
from _utils.utils import download_weights, get_classes, show_config
from _utils.utils_fit import fit_one_epoch
...
if __name__ == "__main__":
    ...
    if True:
        ...
        hrrp_path = '../test_data/MMData2/hrrps.mat'

        hrrp_lines = scio.loadmat(hrrp_path)['aa']

        with open('../test_data/MMData2/label.csv', 'r') as f:
            next(f)
            label_lines = f.readlines()

        train_dataset = CenternetDataset(train_lines, hrrp_lines, label_lines, input_shape, num_classes, train=False)
        ...
        # ----------------------#
        if local_rank == 0:
            eval_callback = EvalCallback(model, backbone, input_shape, class_names, num_classes, val_lines, hrrp_lines, label_lines, log_dir,
                                         Cuda, \
