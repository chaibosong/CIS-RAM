...
from .CenterNet.nets.centernet import CenterNet_HourglassNet, CenterNet_Resnet50
from .CenterNet.nets.centernet_training import get_lr_scheduler, set_optimizer_lr
from .CenterNet.utils.callbacks import LossHistory
from .CenterNet.utils.callbacks import EvalCallback, LossHistory
from .CenterNet.utils.dataloader import CenternetDataset, centernet_dataset_collate
from .CenterNet.utils.utils import download_weights, get_classes, show_config
from .CenterNet.utils.utils_fit import fit_one_epoch

data_path = './datasets/data_for_RECOG'
hrrp_path = 'hrrps'
image_path = 'images'
...
# 目标检测：红外图像单模训练
###############################################################
def train_image(image_path='', label_path='', model1_name='', class_name=None, num_classes=2, epochs=100, batch_size=8,
                learning_rate=5e-4, loadPretrain=0):
    Cuda = True
    # ---------------------------------------------------------------------#
    ...
    model_path = ''
    # model_path = './logs/ep030-loss5.066-val_loss4.386.pth'
    ...
    if backbone == "resnet50":
        ...
        with open('model_data/pth_name.txt', 'r') as f:
            ...
            f.close()
        model_path = os.path.join('weights/singlemodel', model_path)
    else:
    ...
    if True:
        ...
        # ----------------------#
        if local_rank == 0:
            eval_callback = EvalCallback(model, backbone, input_shape, class_names, num_classes, val_lines, log_dir,
                                         Cuda, \
    ...
    print(f'单模态训练完成！权重文件已保存至./weights目录下!')
    print('请继续操作！')
...
if __name__ == '__main__':
    # train = Train()
    # train.start_train()
    train_image('./temp/images', './test_data/label.csv', model1_name='resnet50', class_name=['vehicle', 'building'])